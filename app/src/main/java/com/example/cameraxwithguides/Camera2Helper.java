...
public class Camera2Helper {
    ...
    private Size getBestSupportedSize(List<Size> sizes) {
        ...
        for (int i = (sizes.size() - 1); i >= 0; i--) {
            Log.d(TAG, "width="+sizes.get(i).getWidth() + " height="+sizes.get(i).getHeight());
    ...
    public boolean findCamera(int name){
        CameraManager cameraManager = (CameraManager) context.getSystemService(Context.CAMERA_SERVICE);
        Log.d(TAG, "manager:"+cameraManager + " context:"+context);
        String[] camerasIds;
        try {
        ...
        if (camerasIds.length == 0){
            ...
            for(String id : camerasIds){
                ...
                Log.d(TAG, "size:"+size.length + " id:"+id + " name:"+name);
                if ((name == 0 && size.length==7) || (name==1 && size.length==2)){
                    return true;
                }
            }
        }
...
//            Log.d(TAG, "onImageAvailable:"+image.getFormat());
            lock.lock();
            camera2Listener.onPreview(image);
